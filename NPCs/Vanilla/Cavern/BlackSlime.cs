...
namespace V2.NPCs.Vanilla.Cavern
{
	public static class BlackSlimeStuff
	{
		...
		{
			if (!npc.TryGetGlobalNPC(out BlackSlime BlackSlime))
	...
	public partial class BlackSlime : GlobalNPC
	{
		...
		public override void SetDefaultsFromNetId(NPC npc)
		{
			...
			npc.AsV2NPC().FirstFramePreAIMethod = V2BlackSlimeFirstFrameAI;
			npc.AsV2NPC().NewAIMethod = V2BlackSlimeAI;
			...
			npc.AsSlime().HighJumpXModifier += 0.333f;
			npc.AsSlime().HighJumpYModifier += 0.333f;
			...
			npc.AsPred().CanBeForceFed = CanBlackSlimeBeForceFed;
		...
		public static bool CanBlackSlimeBeForceFed(NPC npc) => true;
