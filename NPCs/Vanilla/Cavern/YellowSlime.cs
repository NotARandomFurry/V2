...
namespace V2.NPCs.Vanilla.Cavern
{
	public static class YellowSlimeStuff
	{
		...
		{
			if (!npc.TryGetGlobalNPC(out YellowSlime YellowSlime))
	...
	public partial class YellowSlime : GlobalNPC
	{
		...
		{
			if (npc.netID != NPCID.YellowSlime)
				return;

			npc.AsV2NPC().Gender = EntityGender.Other;
			npc.AsV2NPC().FirstFramePreAIMethod = V2YellowSlimeFirstFrameAI;
			npc.AsV2NPC().NewAIMethod = V2YellowSlimeAI;
			...
			npc.AsSlime().HighJumpFrequency = 3;
			npc.AsSlime().HighJumpXModifier += 0.25f;
			npc.AsSlime().HighJumpYModifier += 0.4f;
		...
		public static bool CanYellowSlimeBeForceFed(NPC npc) => true;
