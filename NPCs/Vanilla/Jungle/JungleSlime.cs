...
namespace V2.NPCs.Vanilla.Jungle
{
	public static class JungleSlimeStuff
	{
		...
		{
			if (!npc.TryGetGlobalNPC(out JungleSlime JungleSlime))
	...
	public partial class JungleSlime : GlobalNPC
	{
		...
		public override void SetDefaultsFromNetId(NPC npc)
		{
			...
			npc.AsV2NPC().FirstFramePreAIMethod = V2JungleSlimeFirstFrameAI;
			npc.AsV2NPC().NewAIMethod = V2JungleSlimeAI;

			npc.AsSlime().JumpSpeed = new Vector2(5f, 3f);
			...
			npc.AsSlime().HighJumpFrequency = 1;
			npc.AsSlime().HighJumpXModifier += 0.5f;
			npc.AsSlime().HighJumpYModifier += 0.5f;
		...
		public static bool CanJungleSlimeBeForceFed(NPC npc) => true;
		...
		public static double GetDigestionTickRate(NPC npc, PreyData prey) => 0.85;
		public static double GetDigestionTickDamage(NPC npc, PreyData prey)
		{
			double baseDigestionTickDamage = 15.0;
