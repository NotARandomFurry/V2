...
namespace V2.NPCs
{
	public static class ModdedTownNPCAI
	{
		public static void AI_007_TownEntities(NPC npc)
		{
			...
			if (npc.type == NPCID.Mechanic)
			{
				...
				if (Main.projectile.IndexInRange(num3))
				{
					...
					if (projectile.active && projectile.type == ProjectileID.MechanicWrench && projectile.ai[1] == (float)npc.whoAmI)
						flag2 = true;
				}
			...
			bool canBreatheUnderwater = npc.type == NPCID.Turtle || npc.type == NPCID.TurtleJungle || npc.type == NPCID.SeaTurtle || npc.type == ModContent.NPCType<Echo>();
			...
			if (!canBreatheUnderwater && Main.netMode != NetmodeID.MultiplayerClient && !flag3)
			{
				for (int m = 0; m < 200; m++)
				{
					if (!Main.npc[m].active || Main.npc[m].friendly || Main.npc[m].damage <= 0 || !(Main.npc[m].Distance(npc.Center) < num8) || (npc.type == NPCID.SkeletonMerchant && NPCID.Sets.Skeletons[Main.npc[m].type]) || (!Main.npc[m].noTileCollide && !Collision.CanHit(npc.Center, 0, 0, Main.npc[m].Center, 0, 0)) || npc.type == ModContent.NPCType<Echo>())
						continue;
			...
			if (Main.netMode != NetmodeID.MultiplayerClient && npc.isLikeATownNPC && !flag3)
			{
				...
				else if (!NPCID.Sets.IsTownPet[npc.type] && flag26 && npc.ai[0] == 0f && npc.velocity.Y == 0f && Main.rand.NextBool(1200) && (npc.type == NPCID.PartyGirl || (BirthdayParty.PartyIsUp && NPCID.Sets.AttackType[npc.type] == NPCID.Sets.AttackType[NPCID.PartyGirl])))
				{
					int num100 = 300;
					int num101 = 150;
				...
				else if (flag26 && npc.ai[0] == 1f && npc.velocity.Y == 0f && num > 0 && Main.rand.NextBool(num))
				{
					...
					if (flag30)
					{
						...
						{
							if (Main.npc[num112].active && (Main.npc[num112].aiStyle == 7 || Main.npc[num112].aiStyle == -1) && Main.npc[num112].townNPC && Main.npc[num112].ai[0] == 5f && (Main.npc[num112].Bottom + Vector2.UnitY * -2f).ToTileCoordinates() == point)
							{
								flag30 = false;
								break;
							}
			...
			if (npc.type == NPCID.TownSlimeYellow || npc.type == NPCID.BoundTownSlimeYellow)
			{
		...
		public static void AI_007_FindGoodRestingSpot(NPC npc, int myTileX, int myTileY, out int floorX, out int floorY)
		{
			...
			for (int j = 0; j < 200; j++)
			{
				if (Main.npc[j].active && (Main.npc[j].aiStyle == 7 || Main.npc[j].aiStyle == -1) && Main.npc[j].townNPC && Main.npc[j].ai[0] == 5f && (Main.npc[j].Bottom + Vector2.UnitY * -2f).ToTileCoordinates() == point2)
					return;
			}
		...
		public static void AI_007_TryForcingSitting(NPC npc, int homeFloorX, int homeFloorY)
		{
			...
			if (flag)
			{
				...
				for (int i = 0; i < 200; i++)
				{
					if (Main.npc[i].active && (Main.npc[i].aiStyle == 7 || Main.npc[i].aiStyle == -1) && Main.npc[i].townNPC && Main.npc[i].ai[0] == 5f && (Main.npc[i].Bottom + Vector2.UnitY * -2f).ToTileCoordinates() == point)
					{
						flag = false;
						break;
					}
