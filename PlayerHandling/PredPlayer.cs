...
=======
		...
		{
			/*float TumWeight = (float)(player.AsPred().StomachWeight + 1.0);
			float ExtraWeight = 0;
            if (player.AsV2Player().BaeTransformation)
				ExtraWeight += (float)player.AsPred().BaeTransformation_ExtraWeight;
            else if (player.AsV2Player().KroniiTransformation)
                ExtraWeight += (float)player.AsPred().KroniiTransformation_ExtraWeight;
            else if (player.AsV2Player().OllieTransformation)
                ExtraWeight += (float)player.AsPred().OllieTransformation_ExtraWeight;
            else if (player.AsV2Player().SoraTransformation)
                ExtraWeight += (float)player.AsPred().SoraTransformation_ExtraWeight;
            else if (player.AsV2Player().MintTransformation)
                ExtraWeight += (float)player.AsPred().MintTransformation_ExtraWeight;
            player.AsPred().ExcessWeightModifier.ApplyTo(ExtraWeight);*/
...
>>>>>>> Stashed changes
		...
		public static bool CanSwallow(Player pred, Entity prey, bool forced = false)
		{
			...
			//if (pred.AsPred().StomachCapacity != -1 && PreyData.GetPreySize(prey) > (pred.AsPred().StomachCapacity * pred.AsPred().AllowedOverstuffing) - pred.AsPred().StomachFullness && !pred.AsPred().Rose)
			//	return false;
